class LivrosController < ApplicationController
  ...
  def index
    ...
    @livros = Livro.where(filtro).order("titulo").paginate(page: params[:page], per_page: 10);
  end
  ...
  def listar
    ...
    respond_to do |format|
      format.html
      format.pdf do
        pdf = LivrosReport.new(@livros)
        send_data pdf.render, filename: 'LivrosListagem.pdf', :width => pdf.bounds.width,
  ...
  private
    ...
    def livro_params
      params.require(:livro).permit(:titulo, :ano_publicacao, :edicao, :genero, :capa, :editoras_id, :autores_id)
    end
